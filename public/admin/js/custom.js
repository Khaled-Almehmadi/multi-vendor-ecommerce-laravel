...
$("#current_pwd").keyup(function(){
    ...
    $.ajax({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // Fixed missing ]
        },
        type: 'post',
...
$(document).on("click", "#deleteCategoryImage", function () {
    ...
    if(confirm("Are you sure you want to delete the image of this category")){
    var category_id = $(this).data("category-id");
...
$(document).on("click", "#deleteSizeChartImage", function () {
    ...
    if(confirm("Are you sure you want to delete the sizchart image of this category")){
    var category_id = $(this).data("category-id");
    ...
    $.ajax({
        ...
        type: 'post',
        url: '/admin/delete-category-sizechart-image',
        data: {category_id: category_id},
        success: function (resp) {
           if(resp['status']){
...
// Swal.fire('Good job!','u clicked the btn','success');
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (form.length > 0 && form.attr("action") && form.attr("method") === "POST") {
                // Create and append hidden __method input if not present
