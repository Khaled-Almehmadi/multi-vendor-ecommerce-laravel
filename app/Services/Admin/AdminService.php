...
class AdminService {
    ...
    if ($request->hasFile('image')) {
        ...
        if ($image_tmp->isValid()) {
            ...
            $image_path = public_path('admin/images/photos/' . $imageName);  // Use 'public_path()' to save images
...
public function deleteSubadmin($id){
    ...
    return array("message"=>"subadmin deleted succesfully");
}
...
public function updateSubadminStatus($data){
    ...
    if ($status=="Active") {
        ...
        Admin::where('id', $data['subadmin_id'])->update(['status' => 0]);
        return false;
    }
    Admin::where('id', $data['subadmin_id'])->update(['status' => 1]);
    return true;
}
