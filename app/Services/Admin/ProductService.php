...
public function addEditProduct($request) {
    ...
    // Upload  image
if (!empty($data['image_hidden'])) {
    
    $sourcePath = public_path('temp/' . $data['image_hidden']);
    $destinationPath = public_path('front/images/products/' . $data['image_hidden']);
    ...
    $product->image = $data['image_hidden'];
...
// Preserve existing values if no new uploads
...
{
    $imageName = time() . '.' . rand(1111,9999) . '.' . trim($file->getClientOriginalExtension());
