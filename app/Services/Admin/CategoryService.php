    ...
    return [
        ...
        public function updateCategoryStatus($data){
            ...
            if ($status=="Active") {
                ...
                Category::where('id', $data['category_id'])->update(['status' => 0]);
                return false;
            }
            Category::where('id', $data['category_id'])->update(['status' => 1]);
            return true;
        }
        ...
        public function deleteCategoryImage($id){
                ...
                if($image){
                  $imagePath = 'admin/images/categories/'.$image;

                  //we check if the image exsist in path and delete it
         ...
         public function deleteSizeChartImage($id){
                ...
                $image =Category::where('id',$id)->value('size_chart');
              
                if($image){
                  $imagePath = 'admin/images/sizecharts/'.$image;
                ...
                Category::where('id',$id)->update(['size_chart' => null]);
                     return ['status'=>true,'message'=>'Sizechart Image deleted successfully !'];
                }
                

                return ['status'=>false,'message'=>'Sizechart Image not found'];
